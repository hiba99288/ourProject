...
function mapCreator(data ){
    ...
    // Create markers & circles
        data.map( x => {
         ...
         let casedata = []
        fetch('http://localhost:2000/cases')
            .then(res => res.json())
            .then(json=>{
                casedata=json;
                console.log('json', json );
                mapCreator(data,casedata);
            }).catch(err=>console.log(err));
 
            casedata.map( y => {
                
                console.log(y.color+"ppp");
                })
           ...
           var colore="";
            ...
            if(((x.CurrentPositiveCase/x.population)*100)>10)
            {
                colore="red"
              }
            else  if(((x.CurrentPositiveCase/x.population)*100)>=6 && ((x.CurrentPositiveCase/x.population)*100)<=10 )
            {
                colore="orange"
            ...
            else  if( ((x.CurrentPositiveCase/x.population)*100)<6 )
  {
            colore="green"
  }
