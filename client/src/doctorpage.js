...
const useStyles = makeStyles((theme) => ({
  root: { flexGrow: 1, backgroundColor: theme.palette.background.paper,  display: 'flex', height: "1000px", direction: "rtl"},
  ...
  const handleChange = (event, newValue) => { setValue(newValue);};
    return (
       ...
       <Tabs  orientation="vertical" variant="scrollable"  value={value}  onChange={handleChange} aria-label="Vertical tabs example"className={classes.tabs}>
          ...
          <Tab label="انشاء حساب مستخدم" {...a11yProps(1)} />
          <Tab label="التحكم باعدادات الخارطة" {...a11yProps(2)} />
          <Tab label="اضافة مناطق  " {...a11yProps(3)} />
